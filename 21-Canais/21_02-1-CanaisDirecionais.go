...
func main() {
	canal := make(chan int) // Canal Bidirecional (Sem setas. envia e recebe)

	go send(canal) // Precisa rodar de forma concorrente por isso a GoRuoutine

	receive(canal)

}

//Envia a informacao pro canal
// s chan<- int : entra informacao no canal
func send(s chan<- int) {
	s <- 22
}

//Recebe a informacao do canal
// r <-chan int : retira informacao no canal
func receive(r <-chan int) {
	fmt.Println("O valor recebido do canal foi:", <-r)  // <-r : tira do canal informacao
}